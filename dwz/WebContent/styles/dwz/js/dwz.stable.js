		...
		return this.each(function(){
			...
			//加入 开始
			 $("th", thead).click(function () {
	                ...
	                $arr = [],
	                $trs = $grid.find(".gridTbody").find("table tr"),
	                $frg = document.createDocumentFragment(),
	                
	                tableOrderFlag = $th.parent().parent().attr("order");
	                thOrderFlag = $th.attr("order");
	                if(tableOrderFlag == null || tableOrderFlag == "false"){
	                	return;
	                }
	                if(thOrderFlag == null || thOrderFlag == "false"){
	                	return;
	                }
	                ...
	                if ($th.hasClass("selth")) {
	                    ...
	                    $th.toggleClass("asc");
	                    $arr.sort(tabSort($thi + 1));
	                    $(".selth").removeClass();
	                    $th.addClass("selth");
	                }
	                $.each($arr, function (i) { $frg.appendChild($arr[i]) });
	                $grid.find(".gridTbody").find("table").empty().append($frg);
	            });
	            function tabSort(i) {
	                return function (a, b) {
	                    var keya = $(a).find("td:nth-child(" + i + ")").find("div").text();
	                    var keyb = $(b).find("td:nth-child(" + i + ")").find("div").text();
