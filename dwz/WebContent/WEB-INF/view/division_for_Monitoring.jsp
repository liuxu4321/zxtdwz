...
<SCRIPT type="text/javascript" >
	...
	*/
	function zTreeAjax(json){
		...
		var t = $("#tree_for_Monitoring");
		//后台传来的默认选中节点的ID
		var selectedNodeId = $("#selectedNodeId").val();
		t = $.fn.zTree.init(t, setting, json);
		var zTree = $.fn.zTree.getZTreeObj("tree_for_Monitoring");
		//通过ID获取节点实例
	...
	*/
	function nodeOnClick(event, treeId, treeNode, clickFlag) {
		var Node = document.getElementById("hiddenlink_Tree_for_Monitoring");
		//控制程度
		...
		Node.setAttribute("href","${context }/PollutionSourceMonitoringController/init/menuContral?kind="+treeNode.kind+"&id="+treeNode.id+"&attentionCode="+attentionCode+"&kindCode="+kindCode+"&industryCode="+industryCode);
	...
	*/
	function searchTreeNodes(){
		updateNodes(false);
		var zTree = $.fn.zTree.getZTreeObj("tree_for_Monitoring");
		var keyValue = document.getElementById("key_for_Monitoring").value;
	...
	*/
	$(document).ready(function(){
		...
		var kindCode = $("#kindCode").val();
		var industryCode = $("#industryCode").val();
		$(function(){ajaxTodo("${context }/division/init/monitoring?attentionCode="+attentionCode+"&kindCode="+kindCode+"&industryCode="+industryCode,zTreeAjax);});
	});
   ...
   $(document).ready(function(){ 
      init(); 
      $('#key_for_Monitoring').bind('keyup',processKeyup); 
   }); 
   ...
   function init(){ 
      $('#auto_div').hide(); 
      var pos = $('#key_for_Monitoring').position(); 
      var topPosition = pos.top+$('#key_for_Monitoring').height()+10; 
   ...
   function processAjaxRequest(){ 
  	 var inputValue = $('#key_for_Monitoring').val();
   ...
   function getAutoText(){ 
      ...
      if(highLightIndex != -1){ 
          $('#key_for_Monitoring').val($(this).html()); 
          $('#auto_div').html('').hide(); 
      } 
   } 
   ...
   function processEnter(){ 
       if(highLightIndex != -1){ 
          $('key_for_Monitoring').val($('#auto_div').children().eq(highLightIndex).html()); 
          $('#auto_div').html('').hide(); 
       } 
   } 
   ...
   function processKeyUpAndDown(keyCode){ 
       ...
       $('#key_for_Monitoring').val(words.eq(highLightIndex).html()); 
   } 
...
<table style=" width: 100%;" >
	<tr>
		<td width="122px">
			<input class="empty" id="key_for_Monitoring" style="width: 140px;margin-left: 10px;margin-top:6px; ">
		</td>
		<td align="left">
...
<div class="accordionContent">
	<ul id="tree_for_Monitoring" class="ztree" style="width:400px; overflow:auto;"></ul>		
</div>
<a id="hiddenlink_Tree_for_Monitoring" href="" target="ajax" rel="MainBox_Middle" title=""></a>
