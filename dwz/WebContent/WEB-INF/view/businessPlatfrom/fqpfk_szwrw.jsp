...
<script type="text/javascript">
	...
	function editFqpfkSzwrw(){
		...
		if(standardId == null || standardId == ""){
			...
			var $gasOutfallId = $("input[name='gasOutfallId']").val();
			$(function(){ajaxTodo("${context }/businessPlatfrom/gyqy/szwrwEdit?standardId="+standardId+"&outfallId="+$gasOutfallId,searchcallBackFqpfkSzwrwEdit);});
...
<div class="pageContent" width="100%" layoutH="0">
	<div class="panelBar">
		<ul class="toolBar">
			...
			<li><a class="edit" id="xzpfbzA" href="javaScript:choicebzFqpfk();"><span>选择标准</span></a></li>
		</ul>
	</div>
	<form id="szwrwForm" method="post"
		action="${context }/businessPlatfrom/gyqy/szwrwAddSave?navTabId=pfk_szwrw&callbackType=closeCurrent"
			...
			<table style="border: 0; width: 560px;margin-top: 10px;margin-left: 10px;" layoutH="70">
				...
				<tr>
					...
					<td colspan="2" id="pollutantId" style="border-right: 0; border-bottom: 0;">
						...
						<!-- 设置污染物保存用排放口ID -->
						<input type="hidden" name="gasOutfallId" value="${outfallId}" /> 
						<!-- 设置污染物保存用排放标准ID -->
