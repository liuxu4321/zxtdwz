...
//-->
	...
	*	查看选中的污染物的详细信息
	*/
	function fsjkdSzwrwSearch(standardId){
		//废水排放口
		$(function(){ajaxTodo("${context }/businessPlatfrom/pfk/fspfkSzwrwSearch?standardId="+standardId+"&outfallIdsw="+$("#jkd_outfallId_sw").val(),searchcallBackFsjkd);});
	}
	...
	*/
	function addSzwrw(){
		...
		$("#jkd_szwrwWForm").attr("action",
				"${context }/businessPlatfrom/jkd/szwrwAddSaveWater");
		$("#fqwrwszSearchM").html("添加");
	}
	
	/**
	*	查看或修改回调，给界面标签赋值
	*/
	function searchcallBackFsjkd(json){
		...
		document.getElementById("jkd_pollutAlarmMax").innerText = json.ABNORMAL_MAX == null?"":json.ABNORMAL_MAX;
		...
		document.getElementById("jkd_pollutAbnormalMax").innerText = json.ABNORMAL_MAX == null?"":json.ABNORMAL_MAX; 
		 if(json.searchOrEdit == "search" ){
			...
			$("#jkd_szwrwWForm").attr("action",
					"${context }/businessPlatfrom/jkd/szwrwEditAndSaveWater");
			$("#fqwrwszSearchM").html("修改");
		 } 
	}
	...
	*/
	function editFsjkdSzwrw(){
		...
		if(standardId == null || standardId == ""){
			...
			$(function(){
				ajaxTodo("${context }/businessPlatfrom/pfk/szwrwEdit?standardId="
						+standardId+
						"&outfallIdsw="+$('#jkd_outfallId_sw').val(),
						searchcallBackFsjkd);
			});
		}
	}
	...
	*	选择标准执行
	*/
	function choicebzFsjkd(){
		var $pollutId = $("input[name='jkd_pollut.puId']").val();
		if($pollutId == null || $pollutId == ""){
			alertMsg.info("请先选择污染物");
		}else{
			$("#jkd_xzpfbzB").attr("href",
					"${context }/businessPlatfrom/gyqy/szwrwAddChoicebz?pollutId="
							+$pollutId);
			$("#jkd_xzpfbzB").click();
		} 
	}
...
<div class="pageContent" width="100%" layoutH="0">
		...
		<div>
			...
			<div class="panelBar">
				<ul class="toolBar">
					...
					<li><a class="edit" href="javaScript:editFsjkdSzwrw();"><span>修改</span></a></li>
					<li style="display: none;"><a class="delete"
						id="jkd_reflshWrw" target="navTab"
						href="${context }/businessPlatfrom/jkd/fsjkd/szwrw/${monId }"
						rel="pfk_szwrw" title="设置废水监控点污染物"> <span></span></a></li>
				</ul>
			</div>
				...
				<tbody>
					<c:forEach var="item"
						...
						<tr rel="${item.standardId }" target="standardId"
							onclick="fsjkdSzwrwSearch(${item.standardId });">
	...
	<div class="panel open collapse" style="margin-top: 10px;">
		<h1>
			排放口污染物 - <label id="fqwrwszSearchM">添加</label>
		</h1>
		<div>
			<div class="pageContent" width="100%">
				<form id="jkd_szwrwWForm" method="post"
					action="${context }/businessPlatfrom/jkd/szwrwAddSaveWater"
						...
						<table style="border: 0; width: 100%;">
								...
								<td style="border-left: 0; border-bottom: 0;" align="right">
									<div id="jkd_choiseWrwAndbz" style="display: block;">
										<a style="margin-right: 10px; color: blue;"
											...
											rel="xzwrw">选择污染物</a>
										<%-- <a id="xzpfbzA"  style="margin-right:10px;color:blue;" lookupGroup="blowoff" ask="true" max="false" width="900" height="500"
										 href="${context }/businessPlatfrom/gyqy/szwrwAddChoicebz" rel="xzpfbz">选择标准</a> --%>
										<a id="xzpfbzA" style="margin-right: 10px; color: blue;"
											href="javaScript:choicebzFsjkd();">选择标准</a> <a id="jkd_xzpfbzB"
							...
							<tr>
								...
								<td><input id="jkd_standardstandardCodeValue" type="text"
									class="required" name="jkd_blowoff.standardValue"
